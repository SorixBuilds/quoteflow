...
/**
 * `GET /api/v1/leads/[id]` (§21.6) — one lead, only if it belongs to the
 * key's organization. Foreign/absent/malformed ids all 404 identically.
 */
...
export const GET = apiHandler<{ id: string }>(async (req, context) => {
  ...
  return itemResponse(serializeLead(lead));
});
